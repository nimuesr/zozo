...
# --- The scored hit ---------------------------------------------------------
def event_rule_orbs(birth_data, candidate_time: str, event, rules):
    """Diagnostic: for ONE event at ONE candidate time, the best-aspect orb of every
    rule whose category applies -- INCLUDING near-misses beyond max_orb (which the
    scorer would discard). Lets the UI explain *why* an event didn't fire and *how
    close* it came. Returns [(rule, aspect, orb)], not scored."""
    ...
    event_jd = _event_noon_jd(event.event_date)
    transit = {p.name: p.longitude for p in eph.planet_positions(event_jd)}
    arc = solar_arc_degrees(natal_jd, natal["Sun"], event_jd)

    out = []
    for rule in rules:
        if not rule.applies(event.category):
            continue
        moving = transit[rule.point] if rule.technique == "transit" else (natal[rule.point] + arc) % 360.0
        best_aspect, best_orb = None, None
        for a in rule.aspects:
            orb = aspect_orb(moving, target_lon[rule.target], ASPECT_ANGLES[a])
            if best_orb is None or orb < best_orb:
                best_aspect, best_orb = a, orb
        out.append((rule, best_aspect, best_orb))
    return out
